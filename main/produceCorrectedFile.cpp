...
// todo.
// AD 18-10-16. Check for existence of file. See
...
int parseOptions(config &c, int argc, char *argv[]) {
  ...
  if ((argc == 1) ||
      (vm.count(
          "help"))) { // change to 1, argc ==1 means you only passed the file
    std::cout << desc << "\n";
    return 1;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (c.uplinkMax < c.uplinkMin) {
    std::cerr
        << "error in configuration: uplink min is bigger that uplink max\n";
    return 0;
  }
  ...
  std::string location =
      path_head + "SciFiTestbeamAndSimulation/RunNumbers/runNumbers.txt";
  std::cout << "looking for location" << location << std::endl;
  // check if file exists
  if (!file_exists(location)) {
    std::cout << "Something terribly wrong with finding the run numbers"
              << std::endl;
    return 0;
  }
  ...
  c.darkFileName = data_path + "btsoftware_" + std::to_string(calNum.dark) +
                   "_calib_dark_ntuple.root";
  c.ledFileName = data_path + "/btsoftware_" + std::to_string(calNum.led) +
                  "_calib_led_ntuple.root";
  ...
  TString newFileName =
      c.outputLocation + "/corrected/" + removePath(c.file2correct);
