...
void Cluster::Clear() { RelatedChannels.clear(); }

void Cluster::AddChannel(const unsigned int uplNumber,
                         const unsigned int chanNumber, const double adcValue) {
  Channel c;
  c.Uplink = uplNumber;
...
void Cluster::RemoveChannel(const unsigned int chanNumber) {
  for (std::vector<Channel>::iterator chan = RelatedChannels.begin();
       chan != RelatedChannels.end(); ++chan) {
    if (chan->ChannelNumber == chanNumber) {
      RelatedChannels.erase(chan);
      return;
    }
  }
  std::cerr << "Cluster::RemoveChannel: Channel number " << chanNumber
            << " not in cluster -> cannot remove." << std::endl;
}

void Cluster::Resize(const unsigned int newsize) {
  while (RelatedChannels.size() > newsize) {
    std::cout << "current cluster size is " << RelatedChannels.size()
              << " max size is " << newsize << " -> reducing" << std::endl;
    if (RelatedChannels.front().AdcValue > RelatedChannels.back().AdcValue)
      RelatedChannels.pop_back();
    else
      RelatedChannels.erase(RelatedChannels.begin());
  }
}
...
unsigned int Cluster::GetClusterSize() const { return RelatedChannels.size(); }
