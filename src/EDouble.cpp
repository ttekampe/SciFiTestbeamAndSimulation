...
EDouble::EDouble(double _val, double _err) : val(_val), err(_err) {
  if (_err < 0.) err *= (-1.);
}

EDouble::EDouble(const EDouble& a) : val(a.GetVal()), err(a.GetError()) {}

double EDouble::GetVal() const { return val; }

double EDouble::GetError() const { return err; }

void EDouble::SetVal(const double _val) { val = _val; }
...
std::string EDouble::Latex() {
  ...
  //    if(firstRelevantDigit < val_str.size() && firstRelevantDigit <
  //    err_str.size()) firstRelevantDigit+=1; //print 2 relevant digits
  //  }
...
EDouble operator*(const EDouble& lhs, const EDouble& rhs) {
  ...
  double rel_rErr =
      sqrt((lhs.GetError() / lhs.GetVal()) * (lhs.GetError() / lhs.GetVal()) +
...
EDouble operator+(const EDouble& lhs, const EDouble& rhs) {
  double rVal = lhs.GetVal() + rhs.GetVal();
  double rErr =
      sqrt(lhs.GetError() * lhs.GetError() + rhs.GetError() * rhs.GetError());
