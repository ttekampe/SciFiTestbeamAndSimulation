...
std::vector<Cluster> ClusterCreator::FindClustersInEventMax(
    const Event& event, const double neighbour_threshold,
    const double seed_threshold, const double sum_threshold) {
  std::vector<Cluster> foundClusters;

  Cluster currentCluster;
  bool have_cluster{false};

  for (Event::const_iterator chan = event.begin(); chan != event.end();
       ++chan) {
    if (chan->AdcValue >
        neighbour_threshold) {  // look for channels exceeding cluster threshold
      if (!have_cluster) {
        currentCluster = Cluster();
        currentCluster.AddChannel(chan->Uplink, chan->ChannelNumber,
                                  chan->AdcValue);
      } else {
        currentCluster.AddChannel(chan->Uplink, chan->ChannelNumber,
                                  chan->AdcValue);
    ...
    if (chan->AdcValue <= neighbour_threshold && have_cluster) {
...
std::vector<Cluster> ClusterCreator::FindClustersInEventBoole(
  ...
  // Since Digit Container is sorted wrt channelID, clusters are defined
  // searching for bumps of ADC Count
  std::vector<Channel>::const_iterator seedDigitIter = event.begin();

  while (seedDigitIter != event.end()) {
    // loop over digits

    //    std::cout << "Checking channel number " <<
    //    seedDigitIter->ChannelNumber << std::endl;
    ...
    if (seedDigit.AdcValue >= seedThreshold) {
      ...
      //  debug() << " ---> START NEW CLUSTER WITH SEED @ " <<
      //  seedDigit->ChannelNumber << endmsg;

      std::vector<Channel>::const_iterator startDigitIter =
          seedDigitIter;  // begin channel of cluster
      std::vector<Channel>::const_iterator stopDigitIter =
          seedDigitIter;  // end   channel of cluster
      ...
      // map of contributing MCParticles (MCHits) with their relative energy
      // deposit
      ...
      while (((stopDigitIter - startDigitIter) < maxClusterSize) &&
        ...
        if (ContinueStopLoop && ((stopDigitIter + 1) != event.end())) {
          ...
          // next digit should be in the same SiPM, the channel next door, and
          // above adcThreshold
          if ((stopDigit.Uplink == seedDigit.Uplink) &&
              (stopDigit.ChannelNumber ==
               ((*stopDigitIter).ChannelNumber + 1)) &&
            ...
            if (stopDigit.AdcValue > seedDigit.AdcValue) {
              ...
              //          debug() << "  ==> Redefined the seed @ " <<
              //          seedDigit.ChannelNumber << endmsg;

              // set min and max channel of cluster to the new seed (i.e. reset
              // the cluster finding)
              ...
              // IF next digit ADC < current seed, but passes clustering
              // requirements
          ...
          // IF the next digit does not exist in the container (i.e. done with
          // all ClusterCreator)
          ContinueStopLoop = false;
        }

        // So far, we have extended the cluster to the 'right' as far as we
        // could,
        ...
        if (ContinueStartLoop && ((startDigitIter) != event.begin())) {
          ...
          const Channel startDigit =
              *(startDigitIter - 1);  // the 'previous' digit

          // previous digit should be in the same SiPM, the channel next door,
          // above adcThreshold,
          //  and also after the ending channel of the previous cluster
          if ((startDigit.Uplink == seedDigit.Uplink) &&
              (startDigit.ChannelNumber ==
               ((*startDigitIter).ChannelNumber - 1)) &&
      ...
      // MaxClusterSize has been reached, or iterator stop was set due to
      // criteria
      // Cluster spans from startDigitIter to stopDigitIter

      // Check if cluster size < maxWidth. If not: choose highest ADC sum for
      // cluster and shrink
      // This is possible because we check the size, then extend right, then
      // extend left, and repeat.
      while ((stopDigitIter - startDigitIter) >= maxClusterSize) {
        //        debug() << "Cluster size exceeded threshold, Shrinking
        //        cluster." << endmsg;
        if ((*stopDigitIter).AdcValue > (*startDigitIter).AdcValue)
          startDigitIter++;
        else
          stopDigitIter--;
      }
      ...
      lastStopDigitIter =
          stopDigitIter;  // update the 'previous cluster last channel'

      Cluster currentCluster;
      for (std::vector<Channel>::const_iterator iterDigit = startDigitIter;
           iterDigit < (stopDigitIter + 1); ++iterDigit) {
        currentCluster.AddChannel(iterDigit->Uplink, iterDigit->ChannelNumber,
                                  iterDigit->AdcValue);
      }

      // calculate the cluster charge / mean position
      //      debug() << " ---> Done with cluster finding, now calculating
      //      charge / frac.pos" << endmsg;

      //
      // Before Cluster is recorded, check that total ADC > threshold and size >
      // minSize
      ...
      if ((currentCluster.GetSumOfAdcValues() >= sumThreshold) &&
        ...
        // Set the loop iterator to the end digit, to continue looking for next
        // cluster without overlap.
...
std::size_t ClusterCreator::getNumberOfClusters() const {
  return clusters.size();
}
const std::vector<Cluster>& ClusterCreator::getClusters() const {
  return clusters;
}
