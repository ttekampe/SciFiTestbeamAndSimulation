...
void ClusterMonitor::WriteToNtuple(const ClusterCreator& clCreator,
  ...
  resultTree->Branch("chargeWeightedMean", &d_chargeWeightedMean,
                     "chargeWeightedMean/D");

  double d_hitWeightedMean{0.0};
  resultTree->Branch("hitWeightedMean", &d_hitWeightedMean,
                     "hitWeightedMean/D");
  ...
  for (const auto& feature : features) {
    if (feature.second.size() != clCreator.getNumberOfClusters()) {
      std::cerr << "Cannot add feature " << feature.first
                << " because the vector size does not match the number of "
                   "found clusters\n";
      std::cerr << clCreator.getNumberOfClusters() << " clusters and "
                << feature.second.size() << " values for the feature!\n";
      continue;
    }
    std::cout << "Adding feature " << feature.first << " to result tree\n";
    ptrMap[feature.first] = new double();
    resultTree->Branch(feature.first.c_str(), ptrMap[feature.first],
                       (feature.first + "/D").c_str());
  }
